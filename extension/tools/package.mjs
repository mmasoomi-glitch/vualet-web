...
// ── CHECK 4: sidepanel.html references only allowlisted local resources ──────
{
  ...
  // Normalise before comparing: sidepanel.html writes "./sidepanel.css" while the
  // allowlist holds "sidepanel.css". Without this the check fails on correct
  // markup and would train whoever hits it to disable the check.
  const normalise = (r) => r.replace(/^\.\//, "").split(/[?#]/)[0];
  const badRefs = localRefs.filter(r => !ALLOWLIST.includes(normalise(r)));
  if (badRefs.length) {
